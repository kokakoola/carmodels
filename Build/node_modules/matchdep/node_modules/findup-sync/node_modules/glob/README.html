...
<p>```javascript
var glob = require(“glob”)</p>

<p>// options is optional
glob(“<em>*/</em>.js”, options, function (er, files) {
  // files is an array of filenames.
  // If the <code>nonull</code> option is set, and nothing
  // was found, then files is [“<em>*/</em>.js”]
...
<ul>
  ...
  <li><code>cb</code> {Function}
    <ul>
      <li>
        <table>
          <tbody>
            <tr>
              <td><code>err</code> {Error</td>
              <td>null}</td>
            </tr>
          </tbody>
        </table>
      </li>
      <li><code>matches</code> {Array<string>} filenames found matching the pattern</string></li>
    </ul>
  </li>
</ul>
...
<ul>
  <li><code>pattern</code> {String} Pattern to be matched</li>
  <li><code>options</code> {Object}</li>
  <li>return: {Array<string>} filenames found matching the pattern</string></li>
</ul>
...
<ul>
  ...
  <li><code>options</code> {Object}</li>
  <li><code>cb</code> {Function} Called when an error occurs, or matches are found
    <ul>
      <li>
        <table>
          <tbody>
...
you can re-use the statCache to avoid having to duplicate syscalls.</li>
  <li><code>statCache</code> Collection of all the stat results the glob search
performed.</li>
...
values:
    <ul>
      ...
      <li><code>1</code> - Path exists, and is not a directory</li>
      <li><code>2</code> - Path exists, and is a directory</li>
