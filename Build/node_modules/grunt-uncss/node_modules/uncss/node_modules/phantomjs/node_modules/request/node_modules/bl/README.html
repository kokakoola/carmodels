...
<p>```js
const BufferList = require(‘bl’)</p>

<p>var bl = new BufferList()
bl.append(new Buffer(‘abcd’))
bl.append(new Buffer(‘efg’))
bl.append(‘hi’)                     // bl will also accept &amp; convert Strings
bl.append(new Buffer(‘j’))
bl.append(new Buffer([ 0x3, 0x4 ]))</p>

<p>console.log(bl.length) // 12</p>

<p>console.log(bl.slice(0, 10).toString(‘ascii’)) // ‘abcdefghij’
console.log(bl.slice(3, 10).toString(‘ascii’)) // ‘defghij’
console.log(bl.slice(3, 6).toString(‘ascii’))  // ‘def’
console.log(bl.slice(3, 8).toString(‘ascii’))  // ‘defgh’
console.log(bl.slice(5, 10).toString(‘ascii’)) // ‘fghij’</p>

<p>// or just use toString!
console.log(bl.toString())               // ‘abcdefghij\u0003\u0004’
console.log(bl.toString(‘ascii’, 3, 8))  // ‘defgh’
console.log(bl.toString(‘ascii’, 5, 10)) // ‘fghij’</p>

<p>// other standard Buffer readables
...
<p>fs.createReadStream(‘README.md’)
  .pipe(bl(function (err, data) { // note ‘new’ isn’t strictly required
    // <code>data</code> is a complete Buffer object containing the full data
...
const hyperquest = require(‘hyperquest’)
    , bl         = require(‘bl’)
    , url        = ‘https://raw.github.com/rvagg/bl/master/README.md’</p>
...
bl.append(new Buffer(‘j’))</p>

<p>bl.pipe(fs.createWriteStream(‘gibberish.txt’))
```</p>
...
<p><a name="length"></a>
### bl.length
Get the length of the list in bytes. This is the sum of the lengths of all of the buffers contained in the list, minus any initial offset for a semi-consumed buffer at the beginning. Should accurately represent the total number of bytes that can be read from the list.</p>

<hr />
<p><a name="append"></a>
### bl.append(buffer)
<code>append(buffer)</code> adds an additional buffer or BufferList to the internal list.</p>

<hr />
<p><a name="get"></a>
### bl.get(index)
<code>get()</code> will return the byte at the specified index.</p>

<hr />
<p><a name="slice"></a>
### bl.slice([ start, [ end ] ])
<code>slice()</code> returns a new <code>Buffer</code> object containing the bytes within the range specified. Both <code>start</code> and <code>end</code> are optional and will default to the beginning and end of the list respectively.</p>
...
<code>copy()</code> copies the content of the list in the <code>dest</code> buffer, starting from <code>destStart</code> and containing the bytes within the range specified with <code>srcStart</code> to <code>srcEnd</code>. <code>destStart</code>, <code>start</code> and <code>end</code> are optional and will default to the beginning of the <code>dest</code> buffer, and the beginning and end of the list respectively.</p>

<hr />
<p><a name="duplicate"></a>
### bl.duplicate()
<code>duplicate()</code> performs a <strong>shallow-copy</strong> of the list. The internal Buffers remains the same, so if you change the underlying Buffers, the change will be reflected in both the original and the duplicate. This method is needed if you want to call <code>consume()</code> or <code>pipe()</code> and still keep the original list.Example:</p>

<p>```js
var bl = new BufferList()</p>

<p>bl.append(‘hello’)
bl.append(‘ world’)
bl.append(‘\n’)</p>

<p>bl.duplicate().pipe(process.stdout, { end: false })</p>

<p>console.log(bl.toString())
```</p>

<hr />
<p><a name="consume"></a>
### bl.consume(bytes)
<code>consume()</code> will shift bytes <em>off the start of the list</em>. The number of bytes consumed don’t need to line up with the sizes of the internal Buffers—initial offsets will be calculated accordingly in order to give you a consistent view of the data.</p>

<hr />
<p><a name="toString"></a>
### bl.toString([encoding, [ start, [ end ]]])
<code>toString()</code> will return a string representation of the buffer. The optional <code>start</code> and <code>end</code> arguments are passed on to <code>slice()</code>, while the <code>encoding</code> is passed on to <code>toString()</code> of the resulting Buffer. See the <a href="http://nodejs.org/docs/latest/api/buffer.html#buffer_buf_tostring_encoding_start_end">Buffer#toString()</a> documentation for more information.</p>

<hr />
<p><a name="readXX"></a>
### bl.readDoubleBE(), bl.readDoubleLE(), bl.readFloatBE(), bl.readFloatLE(), bl.readInt32BE(), bl.readInt32LE(), bl.readUInt32BE(), bl.readUInt32LE(), bl.readInt16BE(), bl.readInt16LE(), bl.readUInt16BE(), bl.readUInt16LE(), bl.readInt8(), bl.readUInt8()</p>
...
<p>See the <b><code>[Buffer](http://nodejs.org/docs/latest/api/buffer.html)</code></b> documentation for how these work.</p>

<hr />
<p><a name="streams"></a>
### Streams
<strong>bl</strong> is a Node <strong><a href="http://nodejs.org/docs/latest/api/stream.html#stream_class_stream_duplex">Duplex Stream</a></strong>, so it can be read from and written to like a standard Node stream. You can also <code>pipe()</code> to and from a <strong>bl</strong> instance.</p>
...
<p><strong>bl</strong> is Copyright (c) 2013 Rod Vagg <a href="https://twitter.com/rvagg">@rvagg</a> and licenced under the MIT licence. All rights not explicitly granted in the MIT license are reserved. See the included LICENSE.md file for more details.</p>
