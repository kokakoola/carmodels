<p><img src="https://raw.github.com/hueniverse/hawk/master/images/hawk.png" alt="hawk Logo" /></p>

<p><img align="right" src="https://raw.github.com/hueniverse/hawk/master/images/logo.png" /> <strong>Hawk</strong> is an HTTP authentication scheme using a message authentication code (MAC) algorithm to provide partial
HTTP request cryptographic verification. For more complex use cases such as access delegation, see <a href="https://github.com/hueniverse/oz">Oz</a>.</p>

<p>Current version: <strong>1.0</strong></p>

<p><a href="http://travis-ci.org/hueniverse/hawk"><img src="https://secure.travis-ci.org/hueniverse/hawk.png" alt="Build Status" /></a></p>
...
<ul>
  <li><a href="#introduction"><strong>Introduction</strong></a>
    <ul>
      ...
      <li><a href="#protocol-example">Protocol Example</a>
        <ul>
          <li><a href="#payload-validation">Payload Validation</a></li>
          <li><a href="#response-payload-validation">Response Payload Validation</a></li>
        </ul>
      </li>
      <li><a href="#browser-support-and-considerations">Browser Support and Considerations</a></li>
    </ul>
  </li>
</ul>
<p></p>
<ul>
  <li><a href="#single-uri-authorization"><strong>Single URI Authorization</strong></a>
    <ul>
      <li><a href="#bewit-usage-example">Usage Example</a></li>
    </ul>
  </li>
</ul>
<p></p>
<ul>
  <li><a href="#security-considerations"><strong>Security Considerations</strong></a>
    <ul>
      <li><a href="#mac-keys-transmission">MAC Keys Transmission</a></li>
      <li><a href="#confidentiality-of-requests">Confidentiality of Requests</a></li>
      <li><a href="#spoofing-by-counterfeit-servers">Spoofing by Counterfeit Servers</a></li>
      <li><a href="#plaintext-storage-of-credentials">Plaintext Storage of Credentials</a></li>
      <li><a href="#entropy-of-keys">Entropy of Keys</a></li>
      <li><a href="#coverage-limitations">Coverage Limitations</a></li>
      <li><a href="#future-time-manipulation">Future Time Manipulation</a></li>
      <li><a href="#client-clock-poisoning">Client Clock Poisoning</a></li>
      <li><a href="#bewit-limitations">Bewit Limitations</a></li>
      <li><a href="#host-header-forgery">Host Header Forgery</a></li>
    </ul>
  </li>
</ul>
<p></p>
...
<p>Similar to the HTTP <a href="http://www.ietf.org/rfc/rfc2617.txt">Digest access authentication schemes</a>, <strong>Hawk</strong> uses a set of
...
* avoid the exposure of credentials sent to a malicious server over an unauthenticated secure channel due to client
  failure to validate the server’s identity as part of its TLS handshake.</p>
...
a query parameter called <em>bewit</em> (in falconry, a leather strap used to attach a tracking device to the leg of a hawk).</p>
...
<pre><code>// Authenticate incoming request
...
<p>var header = Hawk.client.header(‘http://example.com:8000/resource/1?b=1&amp;a=2’, ‘GET’, { credentials: credentials, ext: ‘some-app-data’ });
...
<pre><code>// Authenticate the server's response
...
<p><strong>Hawk</strong> utilized the <a href="https://github.com/hueniverse/sntp"><strong>SNTP</strong></a> module for time sync management. By default, the local
...
the HTTP <code>Authorization</code> request header field:</p>
...
* <code>hawk.1.payload</code>
* the content-type in lowercase, without any parameters (e.g. <code>application/json</code>)
...
<ul>
  <li>Payload: <code>Thank you for flying Hawk</code></li>
  <li>Content Type: <code>text/plain</code></li>
  <li>Hash (sha256): <code>Yi9LfIIFRtBEPt74PVmbTF/xVAwPn7ub15ePICfgnuY=</code></li>
</ul>
...
<h2>Response Payload Validation</h2>
...
<h2>Browser Support and Considerations</h2>

<p>A browser script is provided for including using a <code>&lt;script&gt;</code> tag in <a href="/lib/browser.js">lib/browser.js</a>.</p>
...
<a href="http://www.w3.org/TR/cors/#access-control-expose-headers-response-header">specifications</a>, it will indicate that these headers
...
<a href="http://www.w3.org/TR/cors/#simple-response-header">“simple response header”</a> which excludes these fields and would prevent the
Hawk client from authenticating the requests.You can read more about the why and how in this
<a href="http://www.html5rocks.com/en/tutorials/cors/#toc-adding-cors-support-to-the-server">article</a></p>

<h1>Single URI Authorization</h1>
...
<pre><code>Hawk.uri.authenticate(req, credentialsFunc, {}, function (err, credentials, attributes) {
...
<h3>Confidentiality of Requests</h3>
...
<h3>Spoofing by Counterfeit Servers</h3>
...
<h3>Plaintext Storage of Credentials</h3>
...
<h3>Future Time Manipulation</h3>
...
* the <code>tsm</code> MAC digest calculated using the same client credentials over the timestamp has been verified.</p>

<h3>Client Clock Poisoning</h3>
...
options are used with <code>server.authenticate()</code>, a malicous client can mint new host names pointing to the server’s IP address and
...
<h3>Where is the protocol specification?</h3>
...
project instead of a standard. In other words, the <a href="/hueniverse/hawk/tree/master/lib">code</a> is the specification. Not sure about
something? Open an issue!</p>

<h3>Is it done?</h3>

<p>At if version 0.10.0, <strong>Hawk</strong> is feature-complete. However, until this module reaches version 1.0.0 it is considered experimental
and is likely to change. This also means your feedback and contribution are very welcome. Feel free to open issues with questions
and suggestions.</p>

<h3>Where can I find <strong>Hawk</strong> implementations in other languages?</h3>

<p><strong>Hawk</strong>’s only reference implementation is provided in JavaScript as a node.js module. However, others are actively porting it to other
platforms. There is already a <a href="https://github.com/alexbilbie/PHP-Hawk">PHP</a>,
<a href="https://github.com/pcibraro/hawknet">.NET</a>, and <a href="https://github.com/wealdtech/hawk">JAVA</a> libraries available. The full list
is maintained <a href="https://github.com/hueniverse/hawk/issues?labels=port">here</a>. Please add an issue if you are working on another
port. A cross-platform test-suite is in the works.</p>

<h3>Why isn’t the algorithm part of the challenge or dynamically negotiated?</h3>
...
<h3>Why is Host and Content-Type the only headers covered by the request MAC?</h3>
...
<h3>Why not just use HTTP Digest?</h3>
...
<h3>Why bother with all this nonce and timestamp business?</h3>
...
<h3>What are <code>app</code> and <code>dlg</code> in the authorization header and normalized mac string?</h3>
...
<h3>What is the purpose of the static strings used in each normalized MAC input?</h3>
...
<h3>Does <strong>Hawk</strong> have anything to do with OAuth?</h3>
...
<p><strong>Hawk</strong> is a derivative work of the <a href="http://tools.ietf.org/html/draft-hammer-oauth-v2-mac-token-05">HTTP MAC Authentication Scheme</a> proposal
...
<p>The <strong>Hawk</strong> logo was created by <a href="http://chriscarrasco.com">Chris Carrasco</a>.</p>
