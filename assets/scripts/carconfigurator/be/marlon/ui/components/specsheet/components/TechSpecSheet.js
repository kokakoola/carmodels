...
(function() {
    ...
    ui.TechSpecSheet = React.createClass(
        {displayName: 'TechSpecSheet',
    ...
    var RowGroup = React.createClass({displayName: 'RowGroup',
        render: function()
        {
            ...
            {
                rows.push(RowItem( {key:i, name:data[i].name, value:data[i].value + ' ' + data[i].unit, categoryId:categoryId} ));
            }
            ...
            if(subCats)
            {
                ...
                {
                    rows.push(RowItem( {key:i + "" + j, name:subCats[j].name, value:subCats[j].value + ' ' + subCats[j].unit, categoryId:categoryId} ));

                    var subCatRows = subCats[j].rows;
                    for(var k = 0; k < subCatRows.length; k++)
                    {
                        rows.push(RowItem( {key:i + "" + j + "" + k, name:subCatRows[k].name, value:subCatRows[k].value + ' ' + subCatRows[k].unit, categoryId:categoryId, indent:true} ));
                    }                    
                }
            }
            ...
            render:function()
            {
                ...
                return(
                    React.DOM.tr( {style:{ display: 'none' }, className:cssClass}, 
                        React.DOM.td(null),
                        React.DOM.td(null, React.DOM.span( {className:"cc-title"}, title)),
                        React.DOM.td(null, React.DOM.span( {className:"cc-value"}, value))
                    )
                );
            }
        }
